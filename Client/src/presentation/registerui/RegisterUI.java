...
public class RegisterUI {
	...
	public void setFrame() throws FontFormatException, IOException {
		...
		//Font nf = setFont("迷你简雪峰", 20); Font nf3 = setFont("迷你简太极", 20);
		 
		// label设置
		JLabel lblNewLabel_1 = new JLabel("用户名:");
		lblNewLabel_1.setHorizontalAlignment(SwingConstants.CENTER);
	    lblNewLabel_1.setFont(new Font("迷你简雪峰",Font.BOLD,20));
		...
		lblNewLabel_2.setFont(new Font("迷你简雪峰",Font.BOLD,20));
		...
		lblNewLabel_3.setFont(new Font("迷你简雪峰",Font.BOLD,20));
		...
		ensureButton.setFont(new Font("迷你简太极",Font.BOLD,20));
		...
		ensureButton.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				if (textField.getText().trim().length() < 1) {
					...
					/*
					 * try { textInfo.setFont(setFont("文鼎CS舒同体", 18)); } catch
					...
					/*
					 * try { passwordInfo.setFont(setFont("文鼎CS舒同体", 18)); }
					...
					passwordInfo1.setFont(new Font("文鼎CS舒同体", Font.PLAIN,18)); 
				...
				if ((password.equals(password2))
					...
					if (backInfo.equals("注册成功")) {
						frame.dispose();
					} else {
						panel.remove(textInfo);
						textInfo = new JLabel("该用户名已被注册");
						textInfo.setBounds(450, 90, 100, 20);
					    textInfo.setFont(new Font("文鼎CS舒同体", Font.PLAIN,18));
						textInfo.setForeground(Color.red);
						panel.add(textInfo);
					}
