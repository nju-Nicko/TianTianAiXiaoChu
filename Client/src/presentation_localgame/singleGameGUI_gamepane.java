...
public void initlizeGamepane(){
	...
	gamepanel.setBounds(85, 100, 450, 450);
					...
					public void mousePressed(MouseEvent e) {
						...
						if(dropdirection==1){
							if(gameRange.gameRange[now_x][now_y]>7){
								//触发道具
								//触发道具的方法调用，
								Game_info gameinfo=blserver.prop_AB(gameRange.gameRange, now_x, now_y);
								...
								if(gameRange.gameRange[now_y][now_x]>7){
									//触发道具
									//触发道具的方法调用，
									Game_info gameinfo=blserver.prop_AB(gameRange.gameRange, now_y, now_x);
									grade+=gameinfo.getGrade();
									lastTwoHitTime=lastDoubleHitTime;
									lastDoubleHitTime=timerObject.getTime();
									doublehitState=false;
 ...
 */private  int imageNumber=0;
	...
	public void paint(Graphics g) {
			...
			if (getIsmovestate()==2) {
				...
				if(gameRange.guiUpdateCount<=0){
					...
					for (int f = 0; f < 9; f++) {
						...
						for (int j = 0; j < 9; j++) {
							// System.out.print("    "+gameRange[f][j]);
							paintGame(g,gameRange.gameRange[f][j],j * onex, f * oney, onex, oney,j * onex+(8-photosize)*onex/16, f * oney+(8-photosize)*oney/16, onex/8*photosize, oney/8*photosize);
		...
		if(gameRange.guiUpdateCount<=0){
			...
			for (int f = 0; f < 9; f++) {
				for (int j = 0; j < 9; j++) {
					if(f==exchangex1&&exchangey1==j){
					paintGame(g,color1,j * onex-(8-Math.abs(8-gameRange.guiUpdateCount))*y12*onex/8, f * oney-(8-Math.abs(8-gameRange.guiUpdateCount))*x12*oney/8, onex, oney,0, 0,0,0);
					}else if(f==exchangex2&&exchangey2==j){
						paintGame(g,color2,j * onex+(8-Math.abs(8-gameRange.guiUpdateCount))*y12*onex/8, f * oney+(8-Math.abs(8-gameRange.guiUpdateCount))*x12*oney/8, onex, oney,0, 0,0,0);
					}else{
						paintGame(g,gameRange.gameRange[f][j],j * onex, f * oney, onex, oney,0, 0,0,0);
					}
					}
	...
	public void paintGame(Graphics g,int picture,int x1,int y1,int width1,int height1,int x2,int y2,int width2,int height2){
		...
		break;
		case 12:java.net.URL imgurl12 = singleGameGUI.class
		...
		break;
		case 13:java.net.URL imgurl13 = singleGameGUI.class
		...
		break;
		case 14:java.net.URL imgurl14 = singleGameGUI.class
		...
		break;
		case 15:java.net.URL imgurl15 = singleGameGUI.class
		...
		break;
		case 16:java.net.URL imgurl16 = singleGameGUI.class
		...
		break;
		case 17:java.net.URL imgurl17 = singleGameGUI.class
		...
		break;
		case 20:java.net.URL imgurl20 = singleGameGUI.class
